...
@njit
def step(sol, t, dt, u, cinvs):
    global timescheme
    if timescheme == 'rk4':
        k1 = dt*dfdt(sol, t, u, cinvs)
        k2 = dt*dfdt(limit(sol+k1/2.), t + dt/2., u, cinvs)
        k3 = dt*dfdt(limit(sol+k2/2.), t + dt/2., u, cinvs)
        k4 = dt*dfdt(limit(sol+k3), t + dt, u, cinvs)
        return sol + k1/6. + k2/3. + k3/3. + k4/6.
    if timescheme == 'euler':
           # [Q, divF] = dfdt_boltzmann(sol, t, u, cinvs)
           # f_prime = sol + Q*dt
           # ffac = np.sum(f_prime, axis=1)/f0
           # for i in range(0, len(f)):
           #     f_prime[i] /= ffac[i]
           # f_next = f_prime - divF*dt
           # return f_next
...
plt.title("Density, Velocity, and Pressure in x at t = 25, BGK")
plt.savefig("rho_U_P_BGK.png")
...
plt.title("Density Gradient in x at t = 25, BGK")
plt.savefig("dPdx_BGK.png")
